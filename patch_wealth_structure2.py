...
grid_start = content.find('            className="grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 px-4 md:px-0"\n          >')
grid_end = content.find('          </motion.div>')

if grid_start != -1 and grid_end != -1:
    new_grid = """            className="flex flex-col gap-8 pb-12 px-4 md:px-0"
          >
              {/* 1. WEALTH HERO */}
              <div className="bg-ui-surface border border-ui-border rounded-3xl p-6 md:p-8 shadow-sm relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`text-sm font-mono font-bold ${summary.totalReturn >= 0 ? 'text-positive' : 'text-negative'} flex items-center bg-positive/10 px-2 py-1 rounded-md`}>
                      ...
                      {summary.totalReturn >= 0 ? '+' : ''}{formatCompactCurrency(summary.totalReturn)} ({summary.totalReturn >= 0 ? '+' : ''}{((summary.totalReturn / summary.investedAmount) * 100 || 0).toFixed(2)}%)
                    </span>
                ...
                <div className="relative z-10 flex flex-row md:flex-col gap-4 md:gap-6 w-full md:w-auto overflow-x-auto custom-scrollbar pb-2 md:pb-0">
                  <div className="min-w-[140px]">
                    ...
                    <p className="text-lg font-mono font-bold text-text-main">{hideBalances ? '••••••••' : formatCompactCurrency(summary.investedAmount)}</p>
                  </div>
                  <div className="min-w-[140px]">
              ...
              <div className="bg-ui-surface border border-ui-border rounded-3xl p-6 shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 pt-6 border-t border-ui-border">
                  <div className="bg-ui-bg p-4 rounded-2xl border border-ui-border">
                    ...
                    <p className="text-sm font-mono font-bold text-text-main">{hideBalances ? '••••••••' : formatCompactCurrency(summary.investedAmount)}</p>
                  </div>
                  <div className="bg-ui-bg p-4 rounded-2xl border border-ui-border">
                    ...
                    <p className={`text-sm font-mono font-bold ${summary.totalReturn >= 0 ? 'text-positive' : 'text-negative'}`}>
                      {summary.totalReturn >= 0 ? '+' : ''}{hideBalances ? '••••••••' : formatCompactCurrency(summary.totalReturn)}
                    </p>
                  </div>
                  <div className="bg-ui-bg p-4 rounded-2xl border border-ui-border">
                    ...
                    <div className="bg-ui-bg border border-ui-border p-5 rounded-2xl hover:border-primary/50 transition-colors cursor-pointer group" onClick={() => navigate('portfolio')}>
                      ...
                      <div className="flex justify-between items-end border-t border-ui-border/50 pt-3">
                        ...
                        <div className="text-right">
                          ...
                          <p className={`text-sm font-mono font-bold ${summary.totalReturn >= 0 ? 'text-positive' : 'text-negative'}`}>
                            {summary.totalReturn >= 0 ? '+' : ''}{hideBalances ? '••••••••' : formatCompactCurrency(summary.totalReturn)}
                            <span className="ml-1 text-[10px]">({summary.totalReturn >= 0 ? '+' : ''}{((summary.totalReturn / summary.investedAmount) * 100 || 0).toFixed(2)}%)</span>
                          </p>
                        </div>
                      </div>
                    </div>
    ...
    print(f"Failed to find grid start ({grid_start}) or end ({grid_end})")
