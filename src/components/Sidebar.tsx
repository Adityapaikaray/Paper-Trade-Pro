    ...
    <div className="hidden md:flex md:w-20 xl:w-64 border-r border-ui-border h-screen flex-col bg-ui-surface z-20 transition-all duration-300 overflow-y-auto custom-scrollbar shrink-0">
      <div className="p-8 md:p-4 xl:p-8 pb-6 sticky top-0 bg-ui-surface z-10 flex justify-center xl:justify-start">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg flex shrink-0 items-center justify-center bg-primary-dark text-white shadow-sm">
      ...
      <nav className="flex-1 px-4 md:px-2 xl:px-4 space-y-1">
        {menuItems.map((item) => (
          ...
          >
            <div className={`shrink-0 transition-all duration-300 ${activeTab === item.id ? 'text-primary-dark' : 'text-text-muted group-hover:text-text-main'}`}>
      ...
      <div className="p-6 md:p-4 xl:p-6 mt-auto">
        <div className="p-5 md:p-0 xl:p-5 rounded-2xl bg-[#EEF2FF] border border-[#E0E7FF] dark:bg-primary/10 dark:border-primary/20 flex flex-col items-center xl:items-start shadow-sm md:bg-none md:border-none xl:bg-linear-to-br xl:border relative group cursor-pointer" onClick={() => openModal('upgrade')}>
          ...
          <div className="hidden xl:flex absolute right-5 bottom-5 w-6 h-6 rounded-full bg-white dark:bg-primary/20 items-center justify-center text-primary-dark shadow-sm group-hover:translate-x-1 transition-transform">
